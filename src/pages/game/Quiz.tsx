...
const Quiz = () => {
    const { id } = useParams();
    const [aircraft, setAircraft] = useState<AircraftInterface | null>(null);
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                if (id) {
                    const data = await fetchAircraftById(parseInt(id));
                    if (mounted) {
                        setAircraft(data);
                    }
                }
    ...
    if (showResult) {
        ...
        const getResultMessage = () => {
            if (percentage >= 80) {
                return "Excellent! You're an aviation expert!";
            }
            if (percentage >= 60) {
                return "Good job! You know your aircraft well.";
            }
            if (percentage >= 40) {
                return "Not bad! Keep learning about aviation.";
            }
